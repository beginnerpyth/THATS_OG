...
def mac():
    ...
    #it doesnt work 
    conn=mysql.connector.connect(host='localhost',user='root',password='password123',database='FULLPROJECT',unix_socket='/tmp/mysql.sock')
    return conn
...
def product(sal:val):
    ...
    conn.commit()##i onlly need when conn.commit when inserting uploading delete
    cur.close()
@bal.get('/SHOW_SALES/')
def showsales(c:int):
